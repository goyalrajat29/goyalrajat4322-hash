...
USERNAME = os.environ.get("GH_PROFILE_USER") or os.environ.get("GITHUB_REPOSITORY_OWNER") or "goyalrajat29"
...
def fetch_days():
    ...
    if not cells:
        print("no calendar cells found -- checking table cells", file=sys.stderr)
    ...
    for td in cells:
        ...
        if not text:
            # fallback to aria-label or inner text
